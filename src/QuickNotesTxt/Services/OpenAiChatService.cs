...
public sealed class OpenAiChatService : IAiChatService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var payload = new ChatCompletionsRequest(model, history.ToList());
    ...
    private sealed record ChatCompletionsRequest(string Model, IReadOnlyList<AiChatMessage> Messages);

    private sealed record ChatCompletionsResponse(IReadOnlyList<Choice>? Choices);

    private sealed record Choice(AiChatMessage? Message);
