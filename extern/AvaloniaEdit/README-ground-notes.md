...
inject a visual-only leading indentation before wrapped continuation indentation is calculated.

## Upgrade procedure

1. Refresh `extern/AvaloniaEdit` from the desired upstream commit.
2. Reapply the `IVisualLineIndentationProvider` and `VisualIndentationElement` patch in `src/AvaloniaEdit/Rendering/`.
